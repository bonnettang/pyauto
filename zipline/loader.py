...
must specify stocks or indexes"""
    ...
    if stocks is not None:
        for stock in stocks:
            logger.info('Loading kkkk stock: {}'.format(stock))
...
def _load_raw_csv_data(stocks, start, end,path):
	data = {}
	names=['Date','open', 'high', 'low', 'close', 'volume','money']
	for stock in stocks:
		logger.info('Loading custom stock: {}'.format(stock))
		tmp_data = pd.read_csv(path+stock+'.csv', names=names,index_col='Date')
		tmp_data.index = pd.DatetimeIndex(tmp_data.index)
		tmp_data.index.name='Data'
		tmp_data.sort_index(ascending=True,inplace=True)
        tmp_data['price']=tmp_data['open']
        data[stock]=tmp_data[start:end]
	return data
	

def load_bars_from_csv(indexes=None,
                         stocks=None,
                         start=None,
                         end=None,
                         adjusted=True):
	...
	"""
    path= 'C:\share\\'
    data = _load_raw_csv_data(stocks, start,end,path=path)
    panel = pd.Panel(data)
    # Rename columns
    panel.minor_axis = ['open', 'high', 'low', 'close', 'volume', 'money','price']
    panel.major_axis = panel.major_axis.tz_localize(pytz.utc)
    return panel
