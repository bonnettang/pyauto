...
def findSpecifiedWindows(hwnd, numChildWindows=70):
    ...
    for window in windows:
        ...
        #print(windowText,windowClass)
        windowContent = dumpSpecifiedWindow(childHwnd)
        #print(windowText,windowClass,len(windowContent))
...
def dumpWindow(hwnd):
    ...
    for window in windows:
        ...
        print('%s %s %s'%(childHwnd, windowText, windowClass))
        if window_content:
            print('window length %d'%(len(window_content)))
...
def sendKeyMsgs(hwnd, key_code_list):
    '''
    模拟按键
    :param hwnd: 窗体句柄
    :param key_code: 按键码，在win32con下，比如win32con.VK_F1
